...
def Html_page(page,all_page):
    ...
    '''
    page_html = []
    start_html = '<a href="/bbschat/index/%d">首页</a>' % (1)
    page_html.append(start_html)
    if page <= 1:
        pre_html = '<a href="#">上一页</a>'
    else:
        pre_html = '<a href="/bbschat/index/%d">上一页</a>' % (page-1)
    ...
    for i in range(start_page,end_page):
        if page == i+1:
            a_html = "<a class='selected' href='/bbschat/index/%d'>%d</a>" % (i + 1, i + 1)
        else:
            a_html = '<a href="/bbschat/index/%d">%d</a>' % (i+1,i+1)
        page_html.append(a_html)
    if page >= all_page:
        nex_html = '<a href="#">下一页</a>'
    else:
        nex_html = '<a href="/bbschat/index/%d">下一页</a>' % (page+1)
    page_html.append(nex_html)
    end_html = '<a href="/bbschat/index/%d">尾页</a>' % (all_page)
